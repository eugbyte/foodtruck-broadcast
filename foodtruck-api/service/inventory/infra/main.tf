...
resource "null_resource" "run_docker_img" {
  provisioner "local-exec" {
    command = "docker compose --file ${local.monorepo_root_path}/docker-compose.yml up --detach postgres foodtruck_inventory"
  }
}

# 2. Push docker image to ECR 
resource "aws_ecr_repository" "foodtruck_inventory" {
  name = "foodtruck-inventory"
}
...
resource "null_resource" "publish_to_ecr" {
  provisioner "local-exec" {
    ...
    command = <<EOF
      ...
      docker tag ${var.image_name} ${aws_ecr_repository.foodtruck_inventory.repository_url}/:latest
      docker push ${aws_ecr_repository.foodtruck_inventory.repository_url}/:latest
     EOF
  }
...
# 4. Create EC2 task
resource "aws_ecs_task_definition" "foodtruck_inventory_task" {
  family                   = "foodtruck-inventory-task" # Naming our first task
  container_definitions    = <<DEFINITION
  [
    {
      "name": "foodtruck-inventory-task",
      "image": "${aws_ecr_repository.foodtruck_inventory.repository_url}",
      "essential": true,
      "portMappings": [
        {
          "containerPort": 8080,
...
module "load_balancer" {
  ...
  name            = "foodtruck_inventory_lb"
  subnets = aws_default_subnet.default_subnet.*.id
}

# 7. Create ECS service
resource "aws_ecs_service" "foodtruck_inventory_service" {
  ...
  task_definition = aws_ecs_task_definition.foodtruck_inventory_task.arn # Referencing the task our service will spin up
  launch_type     = "FARGATE"
  desired_count   = 3 # Setting the number of containers we want deployed to 3

  load_balancer {
    target_group_arn = module.load_balancer.aws_lb_target_group_arn # Referencing our target group
    container_name   = aws_ecs_task_definition.foodtruck_inventory_task.family
