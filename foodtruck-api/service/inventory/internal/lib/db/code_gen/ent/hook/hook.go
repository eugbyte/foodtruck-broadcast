...
// The VendorProductDetailFunc type is an adapter to allow the use of ordinary
// function as VendorProductDetail mutator.
type VendorProductDetailFunc func(context.Context, *ent.VendorProductDetailMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f VendorProductDetailFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.VendorProductDetailMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.VendorProductDetailMutation", m)
}
