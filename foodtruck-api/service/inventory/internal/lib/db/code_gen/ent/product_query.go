...
type ProductQuery struct {
	...
	withVendorProductDetails *VendorProductDetailQuery
...
// QueryVendorProductDetails chains the current query on the "vendor_product_details" edge.
func (pq *ProductQuery) QueryVendorProductDetails() *VendorProductDetailQuery {
	query := (&VendorProductDetailClient{config: pq.config}).Query()
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withVendorProductDetails: pq.withVendorProductDetails.Clone(),
...
// WithVendorProductDetails tells the query-builder to eager-load the nodes that are connected to
// the "vendor_product_details" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithVendorProductDetails(opts ...func(*VendorProductDetailQuery)) *ProductQuery {
	query := (&VendorProductDetailClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withVendorProductDetails = query
	return pq
}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	var (
		...
		_spec       = pq.querySpec()
		loadedTypes = [1]bool{
			pq.withVendorProductDetails != nil,
		}
	)
	...
	if query := pq.withVendorProductDetails; query != nil {
		if err := pq.loadVendorProductDetails(ctx, query, nodes,
			func(n *Product) { n.Edges.VendorProductDetails = []*VendorProductDetail{} },
			func(n *Product, e *VendorProductDetail) {
				n.Edges.VendorProductDetails = append(n.Edges.VendorProductDetails, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (pq *ProductQuery) loadVendorProductDetails(ctx context.Context, query *VendorProductDetailQuery, nodes []*Product, init func(*Product), assign func(*Product, *VendorProductDetail)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(vendorproductdetail.FieldProductID)
	}
	query.Where(predicate.VendorProductDetail(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.VendorProductDetailsColumn), fks...))
	}))
