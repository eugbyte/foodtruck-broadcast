...
func (vq *VendorQuery) QueryVendorProductDetails() *VendorProductDetailQuery {
	query := (&VendorProductDetailClient{config: vq.config}).Query()
...
func (vq *VendorQuery) Clone() *VendorQuery {
	...
	return &VendorQuery{
		...
		withVendorProductDetails: vq.withVendorProductDetails.Clone(),
...
func (vq *VendorQuery) WithVendorProductDetails(opts ...func(*VendorProductDetailQuery)) *VendorQuery {
	query := (&VendorProductDetailClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withVendorProductDetails = query
	return vq
}
...
func (vq *VendorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Vendor, error) {
	var (
		...
		_spec       = vq.querySpec()
		loadedTypes = [1]bool{
			vq.withVendorProductDetails != nil,
		}
	)
	...
	if query := vq.withVendorProductDetails; query != nil {
		if err := vq.loadVendorProductDetails(ctx, query, nodes,
			func(n *Vendor) { n.Edges.VendorProductDetails = []*VendorProductDetail{} },
			func(n *Vendor, e *VendorProductDetail) {
...
func (vq *VendorQuery) loadVendorProductDetails(ctx context.Context, query *VendorProductDetailQuery, nodes []*Vendor, init func(*Vendor), assign func(*Vendor, *VendorProductDetail)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(vendorproductdetail.FieldVendorID)
	}
	query.Where(predicate.VendorProductDetail(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(vendor.VendorProductDetailsColumn), fks...))
	}))
